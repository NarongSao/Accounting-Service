        ...
        creditOfficer: function (branchId) {
            ...
            if (userDoc.roles.Microfis.indexOf("admin-reporter") > -1) {
                ...
                CreditOfficer.find(selector)
                    .forEach(function (obj) {
                        list.push({label: obj._id + " | " + obj.khName, value: obj._id});
                    });
...
export const SelectOptsReport = {
    ...
    client: function () {
        ...
        Client.find({branchId: Session.get("currentBranch")})
            .forEach(function (obj) {
                list.push({label: obj._id + " | " + obj.khSurname + " " + obj.khGivenName, value: obj._id});
            });
    ...
    loanAcc: function () {
        var list = [];
        let data = LoanAcc.find({branchId: Session.get("currentBranch")});
        list.push({label: "(Select One)", value: ""});
        data.forEach(function (value) {
