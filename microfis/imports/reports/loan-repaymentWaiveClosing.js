...
// Method
import {loanRepaymentWaiveClosingReport} from '../../common/methods/reports/loan-repaymentWaiveClosing';
import {SelectOptMethods} from '../../common/methods/select-opts.js';

// Schema
import {LoanRepaymentWaiveClosingSchema} from '../../common/collections/reports/loan-repaymentWaiveClosing';

// Page
import './loan-repaymentWaiveClosing.html';

// Declare template
let indexTmpl = Template.Microfis_repaymentWaiveClosingReport,
    tmplPrintData = Template.Microfis_repaymentWaiveClosingReportPrintData;
// Form state
...
indexTmpl.onCreated(function () {
    createNewAlertify('Microfis_repaymentWaiveClosingReport');
    this.autorun(() => {
        // Check form data
        if (formDataState.get()) {
            ...
            loanRepaymentWaiveClosingReport.callPromise({params: params})
                .then((result) => {
...
indexTmpl.helpers({
    schema(){
        return LoanRepaymentWaiveClosingSchema;
    },
...
indexTmpl.events({
    'click .fullScreen'(event, instance){
        alertify.Microfis_repaymentWaiveClosingReport(fa('', ''), renderTemplate(tmplPrintData)).maximize();
    },
...
AutoForm.addHooks('Microfis_repaymentWaiveClosingReport', hooksObject);
