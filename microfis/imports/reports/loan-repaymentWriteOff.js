...
// Method
import {loanRepaymentWriteOffReport} from '../../common/methods/reports/loan-repaymentWriteOff';
import {SelectOptMethods} from '../../common/methods/select-opts.js';

// Schema
import {LoanRepaymentWriteOffSchema} from '../../common/collections/reports/loan-repaymentWriteOff';

// Page
import './loan-repaymentWriteOff.html';

// Declare template
let indexTmpl = Template.Microfis_repaymentWriteOffReport,
    tmplPrintData = Template.Microfis_repaymentWriteOffReportPrintData;
// Form state
...
indexTmpl.onCreated(function () {
    createNewAlertify('Microfis_repaymentWriteOffReport');
    this.autorun(() => {
        // Check form data
        if (formDataState.get()) {
            ...
            loanRepaymentWriteOffReport.callPromise({params: params})
                .then((result) => {
...
indexTmpl.helpers({
    schema(){
        return LoanRepaymentWriteOffSchema;
    },
...
indexTmpl.events({
    'click .fullScreen'(event, instance){
        alertify.Microfis_repaymentWriteOffReport(fa('', ''), renderTemplate(tmplPrintData)).maximize();
    },
...
AutoForm.addHooks('Microfis_repaymentWriteOffReport', hooksObject);
