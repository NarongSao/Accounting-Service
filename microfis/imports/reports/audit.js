...
// Method
import {auditReport} from '../../common/methods/reports/audit.js';
import {SelectOptMethods} from '../../common/methods/select-opts.js';

// Schema
import {AuditSchema} from '../../common/collections/reports/audit.js';

// Page
import './audit.html';

// Declare template
let indexTmpl = Template.Microfis_auditReport,
    tmplPrintData = Template.Microfis_auditReportPrintData;
// Form state
...
indexTmpl.onCreated(function () {
    createNewAlertify('Microfis_auditReport');
    this.autorun(() => {
        // Check form data
        if (formDataState.get()) {
            ...
            auditReport.callPromise({params: params})
                .then((result) => {
...
indexTmpl.events({
    'click .fullScreen'(event, instance){
        alertify.Microfis_auditReport(fa('', ''), renderTemplate(tmplPrintData)).maximize();
    },
...
AutoForm.addHooks('Microfis_auditReport', hooksObject);
