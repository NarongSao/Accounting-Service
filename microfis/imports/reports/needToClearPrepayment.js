...
// Method
import {needToClearPrepaymentReport} from '../../common/methods/reports/needToClearPrepayment';
import {SelectOptMethods} from '../../common/methods/select-opts.js';

// Schema
import {NeedToClearPrepaymentSchema} from '../../common/collections/reports/needToClearPrepay';

// Page
import './needToClearPrepayment.html';

// Declare template
let indexTmpl = Template.Microfis_needToClearPrepaymentReport,
    tmplPrintData = Template.Microfis_needToClearPrepaymentReportPrintData;
// Form state
...
indexTmpl.onCreated(function () {
    createNewAlertify('Microfis_needToClearPrepaymentReport');
    this.autorun(() => {
        // Check form data
        if (formDataState.get()) {
            ...
            needToClearPrepaymentReport.callPromise({params: params})
                .then((result) => {
...
indexTmpl.helpers({
    schema(){
        return NeedToClearPrepaymentSchema;
    },
...
indexTmpl.events({
    'click .fullScreen'(event, instance){
        alertify.Microfis_needToClearPrepaymentReport(fa('', ''), renderTemplate(tmplPrintData)).maximize();
    },
...
AutoForm.addHooks('Microfis_needToClearPrepaymentReport', hooksObject);
