...
export let checkRepaymentProvision = new ValidatedMethod({
    name: 'microfis.checkRepaymentProvision',
    mixins: [CallPromiseMixin],
    validate: new SimpleSchema({
        loanAccId: {
            type: String
        ...
        if (!this.isSimulation) {
            ...
            scheduleDoc.forEach((o) => {
                ...
                if (checker.detailOnRepaymentDocExist) {
                    ...
                    let detailOnRepaymentDoc = [];
                    o.repaymentDoc.detail.forEach(function (obj) {
                        if (loanAccDoc.status == "Close" && lastRepayment._id == obj.repaymentId && moment(checkDate).startOf("day").toDate().getTime() == moment(loanAccDoc.closeDate).startOf("day").toDate().getTime()) {
                            if (obj.repaidDate.getTime() <= moment(checkDate).startOf("day").toDate().getTime()) {
                                detailOnRepaymentDoc.push(obj);
                            }
                        } else {
                            if([31,61,91].indexOf(obj.numOfDayLate)>-1 && loanAccDoc.accountType=="IL"){
                                if (obj.repaidDate.getTime() <= moment(checkDate).startOf("day").toDate().getTime()) {
                                    detailOnRepaymentDoc.push(obj);
                                }
                            }else if ([31,91,361].indexOf(obj.numOfDayLate)>-1 && loanAccDoc.accountType=="GL"){
                                if (obj.repaidDate.getTime() <= moment(checkDate).startOf("day").toDate().getTime()) {
                                    detailOnRepaymentDoc.push(obj);
                                }
                            }else {
                                if (obj.repaidDate.getTime() <= moment(checkDate).endOf("day").toDate().getTime()) {
                                    detailOnRepaymentDoc.push(obj);
                                }
                            }
                        }
            ...
            scheduleDoc.forEach(function (obj) {
                ...
                if (obj.repaymentDoc) {
                    ...
                    obj.repaymentDoc.detail.forEach(function (newDoc) {
                        ...
                        if (loanAccDoc.status == "Close" && moment(checkDate).startOf("day").toDate().getTime() == moment(loanAccDoc.closeDate).startOf("day").toDate().getTime()) {
                            if (lastRepayment._id == newDoc.repaymentId) {
                                if (newDoc.repaidDate.getTime() <= moment(checkDate).startOf("day").toDate().getTime()) {
                                    balanceUnPaid -= newDoc.principalPaid;
                                    interestUnPaid -= newDoc.interestPaid;
                                    feeOnPaymentUnPaid -= newDoc.feeOnPaymentPaid;
                                }
                             ...
                             if (moment(lastDate).startOf("day").toDate().getTime() == moment(newDoc.repaidDate).startOf("day").toDate().getTime()) {
                                if (newDoc.repaidDate.getTime() <= moment(checkDate).startOf("day").toDate().getTime()) {
                                    balanceUnPaid -= newDoc.principalPaid;
                                    interestUnPaid -= newDoc.interestPaid;
                                    feeOnPaymentUnPaid -= newDoc.feeOnPaymentPaid;
                                    ...
                                    if([31,61,91].indexOf(newDoc.numOfDayLate)>-1 && loanAccDoc.accountType=="IL"){
                                        if (newDoc.repaidDate.getTime() <= moment(checkDate).startOf("day").toDate().getTime()) {
                                            balanceUnPaid -= newDoc.principalPaid;
                                            interestUnPaid -= newDoc.interestPaid;
                                            feeOnPaymentUnPaid -= newDoc.feeOnPaymentPaid;
                                        }
                                    }else if ([31,91,361].indexOf(newDoc.numOfDayLate)>-1 && loanAccDoc.accountType=="GL"){
                                        if (newDoc.repaidDate.getTime() <= moment(checkDate).startOf("day").toDate().getTime()) {
                                            balanceUnPaid -= newDoc.principalPaid;
                                            interestUnPaid -= newDoc.interestPaid;
                                            feeOnPaymentUnPaid -= newDoc.feeOnPaymentPaid;
                                        ...
                                        if(lastRepaymentId==newDoc.repaymentId && lastRepaymentId != "" && loanAccDoc.status=="Active" && totalScheduleDue.numOfDayLate>30){
                                            if (newDoc.repaidDate.getTime() <= moment(checkDate).startOf("day").toDate().getTime()) {
                                                balanceUnPaid -= newDoc.principalPaid;
                                                interestUnPaid -= newDoc.interestPaid;
                                                feeOnPaymentUnPaid -= newDoc.feeOnPaymentPaid;
                        ...
                        if(loanAccDoc.status=="Active"){
                            lastRepaymentId=newDoc.repaymentId;
                        }
                        lastDate = newDoc.repaidDate;
                        lastRepaidDate = newDoc.repaidDate;

                    })
                    penaltyTotal += obj.repaymentDoc.totalPenaltyPaid;
                }else {
                    if(loanAccDoc.status=="Active"){
                        if([31,61,91].indexOf(totalScheduleDue.numOfDayLate)>-1 && loanAccDoc.accountType=="IL"){
                            if (moment(lastRepaidDate).startOf("day").toDate().getTime() == moment(checkDate).startOf("day").toDate().getTime()) {
                                balanceUnPaid -= obj.principalDue;
                                interestUnPaid -= obj.interestDue;
                                feeOnPaymentUnPaid -= obj.feeOnPaymentDue;
                            }
                        }else if ([31,91,361].indexOf(totalScheduleDue.numOfDayLate)>-1 && loanAccDoc.accountType=="GL"){
                            if (moment(lastRepaidDate).startOf("day").toDate().getTime() == moment(checkDate).startOf("day").toDate().getTime()) {
                                balanceUnPaid -= obj.principalDue;
                                interestUnPaid -= obj.interestDue;
                                feeOnPaymentUnPaid -= obj.feeOnPaymentDue;
